...
fn write_managed_script(jar_dir: &Path, ram_mb: u64) -> Result<PathBuf, String> {
    #[cfg(windows)]
    ...
    #[cfg(windows)]
    let content = format!(
        "@echo off\r\nrem Generated by ONEPIXEL Manager\r\ncd /d \"%~dp0\"\r\njava -Xms{ram_mb}M -Xmx{ram_mb}M {JVM_FLAGS} -jar server.jar --nogui\r\nexit /b %errorlevel%\r\n"
    );
    #[cfg(not(windows))]
    let content = format!(
        "#!/bin/sh\n# Generated by ONEPIXEL Manager\ncd \"$(dirname \"$0\")\"\nexec java -Xms{ram_mb}M -Xmx{ram_mb}M {JVM_FLAGS} -jar server.jar --nogui\n"
    );
...
pub async fn start_server(
    ...
    let (script, workdir) = match find_server_jar(&dir) {
        Some(jar_dir) => {
            ...
            (
                write_managed_script(&jar_dir, ram)?,
                jar_dir,
            )
        }
        None => {
